...
class CParser():
    def __init__(self, tokens) -> None:
        self.pg = ParserGenerator(tokens)       # парсер, который может обрабатывать поток лексем (токенов) и строить синтаксическую структуру на основе определенных грамматических правил

        self.tabulate_compound = lambda compound: tab.join(compound.splitlines(True))   # форматирование блоков кода с использованием отступа (четыре пробела)                                                                                                                                                      блоков кода с использованием отступа (четыре пробела


    def parse(self):
        #правило разбора для парсера
        @self.pg.production("main : compound_stmt")         
        def main(p) -> str:
            return '\n'.join(p)                             # возвращает строку, которая объединяет элементы p с помощью символов новой строки


        #правила разбора для парсера
        ...
        # @self.pg.production("compound_stmt : func_stmt")
        ...
        # @self.pg.production("compound_stmt : func_stmt compound_stmt")
        def compound_stmt(p):
        ...
        # #правила разбора для парсера
        # @self.pg.production("func_stmt : tkInteger compound_stmt tkORoundBr compound_stmt tkCRoundBr tkOCurlyBr compound_stmt tkCCurlyBr")
        # @self.pg.production("func_stmt : tkVoid compound_stmt tkORoundBr compound_stmt tkCRoundBr tkOCurlyBr compound_stmt tkCCurlyBr")
        # @self.pg.production("func_stmt : tkBoolean compound_stmt tkORoundBr compound_stmt tkCRoundBr tkOCurlyBr compound_stmt tkCCurlyBr")
        # @self.pg.production("func_stmt : tkString compound_stmt tkORoundBr compound_stmt tkCRoundBr tkOCurlyBr compound_stmt tkCCurlyBr")
        # @self.pg.production("func_stmt : tkFloat compound_stmt tkORoundBr compound_stmt tkCRoundBr tkOCurlyBr compound_stmt tkCCurlyBr")
        # def func_stmt(p):
        #     match p:
        #         case Token('tkInteger') as tkInteger, compound, Token('tkORoundBr'), compound, oken('tkOCurlyBr'), compound, Token('tkCCurlyBr'):
        #             result = f'def {compound} ({compound}):\n{tab}{self.tabulate_compound(compound)}\n'
        #         case Token('tkVoid') as tkInteger, compound, Token('tkORoundBr'), compound, oken('tkOCurlyBr'), compound, Token('tkCCurlyBr'):
        #             result = f'def {compound} ({compound}):\n{tab}{self.tabulate_compound(compound)}\n'
        #         case Token('tkBoolean') as tkInteger, compound, Token('tkORoundBr'), compound, oken('tkOCurlyBr'), compound, Token('tkCCurlyBr'):
        #             result = f'def {compound} ({compound}):\n{tab}{self.tabulate_compound(compound)}\n'
        #         case Token('tkString') as tkInteger, compound, Token('tkORoundBr'), compound, oken('tkOCurlyBr'), compound, Token('tkCCurlyBr'):
        #             result = f'def {compound} ({compound}):\n{tab}{self.tabulate_compound(compound)}\n'
        #         case Token('tkFloat') as tkInteger, compound, Token('tkORoundBr'), compound, oken('tkOCurlyBr'), compound, Token('tkCCurlyBr'):
        #             result = f'def {compound} ({compound}):\n{tab}{self.tabulate_compound(compound)}\n'


        # Правило для разбора определения функции
        @self.pg.production("function_definition : return_type tkID tkORoundBr parameter_list tkCRoundBr tkOCurlyBr compound_stmt tkCCurlyBr")
        def function_definition(p):
            '''
                Разбирает определение функции, включая возвращаемый тип, имя функции,
                параметры и тело функции.
            '''
            # Извлечение элементов из структуры
            match p:
                case return_type, Token('tkID') as function_name, Token('tkORoundBr'), parameters, Token('tkCRoundBr'), Token('tkOCurlyBr'), body, Token('tkCCurlyBr'):
                    # Форматируем результат
                    formatted_body = '\n'.join([f'    {line}' for line in body.splitlines()])
                    result = f"def {function_name.value}({parameters}):\n{tab}{formatted_body}\n"
                    
            return result

        # Правило для возвращаемого типа
        @self.pg.production("return_type : tkVoid")
        @self.pg.production("return_type : tkInteger")
        @self.pg.production("return_type : tkFloat")
        @self.pg.production("return_type : tkBoolean")
        @self.pg.production("return_type : tkString")
        def return_type(p):
        ...
        # Правило для списка параметров
        @self.pg.production("parameter_list : ")
        @self.pg.production("parameter_list : parameter_stmt")
        @self.pg.production("parameter_list : parameter_stmt tkComma parameter_list")
        def parameter_list(p):
            match p:
                ...
                case [param, _, param_list]:
                    return f"{param}, {param_list}"

        # Правило для отдельного параметра
        @self.pg.production("parameter_stmt : type_stmt tkID")
        def parameter_stmt(p):
            match p:
                case [type_token, Token('tkID') as param_name]:
                    return f"{type_token} {param_name.value}"
        ...
        #правило разбора для парсера
        @self.pg.production("function_stmt : input_stmt")
        def function_stmt(p):
            '''Ввод'''
            match p:
                case [INPUT]:
        ...
        #правило разбора для парсера
        @self.pg.production("cin_stmt : tkinop expression_stmt cin_stmt")
        @self.pg.production("cin_stmt : tkinop expression_stmt")
        def cin_stmt(p):
            '''Ввод'''
            match p:
                
                case Token('tkinop'), content:
                    result = f"{content}"
                case Token('tkinop'), content, cin:
                    result = f"{content}, {cin}"
        ...
        #правило разбора для парсера
        @self.pg.production("input_stmt : tkInput tkORoundBr tkCRoundBr ")
        @self.pg.production("input_stmt : tkInput tkORoundBr expression_stmt tkComma expression_stmt tkCRoundBr ")
        @self.pg.production("input_stmt : tkInput cin_stmt")
        def input_stmt(p):
            '''Функция вывода данных'''
            match p:
                case Token('tkInput'), Token('tkORoundBr'), Token('tkCRoundBr'):
                    result = 'input(\n)'

                case Token('tkInput'), Token('tkORoundBr'), pattern, Token('tkComma'), content, Token('tkCRoundBr'):
                    result = f"input(\"{content}\")"

                case Token('tkInput'), cin:
                    result = f"input({cin})"
            return result            
